...
neighbors_t get_legal_neighbors_student(const map_t map, const Coordinates position);

// Student functions
extern uint32_t discrete_distance_asm(const uint32_t a, const uint32_t b);
extern uint32_t hamming_distance_asm(const uint32_t a, const uint32_t b);
...
extern uint32_t get_directions_asm(const char *map, const uint32_t position);
